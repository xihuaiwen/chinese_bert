...
class Bert(Model):
    ...
    def encoder_scope(self, layer_index):
        ...
        # Throughput optimisation for MLM/NSP Inference - move a layer to the next IPU
        # to balance the pipeline
        if self.config.inference and self.config.task == "PRETRAINING" \
                and self.config.squeeze_model and self.config.hidden_size == 768:
            layers_to_move = [5] if self.config.sequence_length == 128 else [4, 5]
            if layer_index in layers_to_move:
                ipu += 1
