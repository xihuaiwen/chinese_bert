...
class Attention(Block):
    def __init__(self,
        ...
        if prefetch_masks:
            ...
            self.mask_ping_pong_phase = scope_provider.get_scope('Mask', 'prev').ping_pong_phase % 2
            self.mask_scope = scope_provider.get_scope('Mask',
                                                       self.mask_ping_pong_phase,
...
class MaskLM(Block):
    def __init__(self,
        ...
        super(MaskLM, self).__init__(params=[],
                                     scope=scope_provider.get_scope(name=f'MLM/{name}',
        ...
        self.dense = Dense(scope_provider.get_scope("Projection", self.scope.ping_pong_phase),
                           hidden_size,
                           vocab_size,
                           split=None,
                           activation='relu',
...
class NextSentencePred(Block):
    def __init__(self, name, batch_size, sequence_length, hidden_size,
                 **kwargs):
        scope_provider = kwargs['scope_provider']
        scope = scope_provider.get_scope(name, ping_pong_phase='next')
        params = []
        ...
        self.pooler = Dense(scope_provider.get_scope(
            "Pool", ping_pong_phase=self.scope.ping_pong_phase),
        ...
        self.classifier = Dense(scope=scope_provider.get_scope(
            "Classifier", ping_pong_phase=self.scope.ping_pong_phase),
            input_dim=hidden_size,
            output_dim=2,
            split=None,
            **kwargs)
...
class SquadProjection(Block):
    def __init__(self, name, batch_size, sequence_length, hidden_size,
        ...
        self.hidden_size = hidden_size
        self.classifier = Dense(scope=scope_provider.get_scope(
            name, ping_pong_phase=self.scope.ping_pong_phase),
            input_dim=hidden_size,
            output_dim=2,
            split=None,
            **kwargs)
