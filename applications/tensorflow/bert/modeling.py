    ...
    def __init__(self,
        ...
        Args:
            ...
            use_qkv_bias: Whether to use bias in QKV calculation of attention layer.
                This is for model optimization.
            matmul_serialize_factor: The factor to serialize matmul. This can decrease memory footprint.
            dtype: Data type.
        """
...
class BertModel(object):
    ...
    def embeddings_layer(self, input_ids, input_mask, segment_ids):
        ...
        word_embeddings = self.embedding(
            input_ids, self.bert_config.vocab_size, name="word_embeddings", num_splits=self.bert_config.matmul_serialize_factor)
    ...
    def self_attention(self, input_tensor, mask=None):
        ...
        qkv_weight = tf.get_variable(
            ...
            shape=[num_attention_heads*size_per_head,
                   3*num_attention_heads*size_per_head],
            initializer=create_initializer(self.bert_config.initializer_range),
            trainable=True)
    ...
    def attention_projection(self, input_tensor, attention_output):
        with tf.variable_scope("projection"):
            ...
            attention_output = dropout(
                attention_output, self.bert_config.hidden_dropout_prob)
    ...
    def feed_forward(self, attention_output):
        ...
        with tf.variable_scope("output"):
            feed_farward_output = tf.layers.dense(
                intermediate_output,
                self.bert_config.hidden_size,
                kernel_initializer=create_initializer(self.bert_config.initializer_range))
            feed_farward_output = dropout(
                feed_farward_output, self.bert_config.hidden_dropout_prob)
            feed_farward_output = layer_norm(
                feed_farward_output + attention_output)
        return feed_farward_output
    ...
    def encoder(self, input_tensor, attention_mask, masked_lm_positions=None):
        ...
        @ipu.function
        def outlined_encoder(input_tensor, attention_mask):
        ...
        input_tensor = outlined_encoder(input_tensor, attention_mask)
    ...
    def embedding_postprocessor_layer(self,
        ...
        with tf.variable_scope("bert"):
            with tf.variable_scope("embeddings"):
                ...
                segment_embeddings = tf.matmul(seg_onehot, seg_weights)
                # full_embeddings = tf.add(word_embeddings, position_embeddings)
                full_embeddings = word_embeddings + segment_embeddings
                
                full_position_embeddings = tf.get_variable(
                    dtype=self.bert_config.dtype,
                    name="position_embeddings",
                    shape=[self.bert_config.max_position_embeddings, self.bert_config.hidden_size],
                    initializer=create_initializer(self.bert_config.initializer_range))
                position_embeddings = tf.slice(full_position_embeddings, [0, 0],
                                     [_seq_len, -1])
                num_dims = len(word_embeddings.shape.as_list())
                position_broadcast_shape = []
                for _ in range(num_dims - 2):
                    position_broadcast_shape.append(1)
                position_broadcast_shape.extend([_seq_len, self.bert_config.hidden_size])
                position_embeddings = tf.reshape(position_embeddings,
                                       position_broadcast_shape)
                full_embeddings += position_embeddings

                full_embeddings = layer_norm_and_dropout(
                    full_embeddings, self.bert_config.hidden_dropout_prob)
                attention_mask = create_attention_mask_from_input_mask(
                    input_ids, input_mask, self.bert_config.dtype)
                return {"input_tensor": full_embeddings,
    ...
    def get_next_sentence_output_layer(self,
        ...
        with tf.variable_scope("cls/predictions/transform"):
            ...
            dtype=self.bert_config.dtype)
        with tf.variable_scope("cls/predictions"):
            logits = tf.nn.bias_add(mlm_logits, output_bias)
...
def gather_indexes(sequence_tensor, positions):
    ...
    output_tensor = ipu.ops.embedding_ops.embedding_lookup(
        flat_sequence_tensor, flat_positions, serialization_factor=4)
