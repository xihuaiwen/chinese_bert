...
def synthetic_pretraining_dataset(opts):
    ...
    _input_ids = np.random.randint(
        vocab_size, size=seq_length).astype(np.int32)
    _input_mask = np.random.randint(2, size=seq_length).astype(np.int32)
    _segment_ids = np.random.randint(2, size=seq_length).astype(np.int32)
    _masked_lm_positions = np.random.randint(
        seq_length, size=max_predictions_per_seq).astype(np.int32)
    _masked_lm_ids = np.random.randint(vocab_size,
                                       size=max_predictions_per_seq).astype(
                                           np.int32)
    _masked_lm_weights = np.random.randint(
        2, size=max_predictions_per_seq).astype(np.float32)
    _next_sentence_labels = np.random.randint(2, size=1).astype(np.int32)
    ...
    ds = tf.data.Dataset.from_tensors(name_dict)
    dataset = ds.cache()
...
def get_pretraining_dataset(opts, data_type, is_training=True, num_cpu_threads=4):
    if is_training:
        input_file = opts['train_file']
    else:
        input_file = opts['test_file']
    ...
    if is_training:
        ...
        # d = d.repeat(opts['dataset_repeat'])
    else:
...
def get_squad_dataset(opts, is_training):
    ...
    if is_training:
        name_to_features["start_positions"] = tf.FixedLenFeature([1], tf.int64)
        name_to_features["end_positions"] = tf.FixedLenFeature([1], tf.int64)
    else:
        name_to_features["start_positions"] = tf.FixedLenFeature(
            [1], tf.int64, default_value=-99)
    ...
    if is_training:
        ...
        name = str(opts['seq_length']) + '_' + str(
            opts['doc_stride']) + '_' + str(
                opts['max_query_length']) + "train.tf_record"
        filename = os.path.join(opts['tfrecord_dir'], name)
        input_file = opts['train_file']
        ...
        name = str(opts['seq_length']) + '_' + str(
            opts['doc_stride']) + '_' + str(
                opts['max_query_length']) + "eval.tf_record"
        filename = os.path.join(opts['tfrecord_dir'], name)
        input_file = opts['predict_file']

    if not os.path.exists(filename):  # and not is_training:
        print('prerpocess data: {}'.format(filename))
    ...
    if is_training:
        ...
        d = d.repeat(math.ceil(opts['epochs']))
    ...
    if opts['task'] == 'pretraining':
        if opts['synthetic_data']:
            ...
            return get_pretraining_dataset(
                ...
                data_type,
                is_training,
                num_cpu_threads=int(opts['parallell_io_threads']))
    elif opts['task'] == 'SQuAD':
        return get_squad_dataset(opts, is_training)
    else:
        raise ValueError(
            "Conflict options between synthetic_data and input_file")


def set_defaults(opts):
    if opts['synthetic_data'] and opts['train_file']:
        raise ValueError(
            "Conflict options between synthetic_data and input_file")
    if not opts['synthetic_data'] and not opts['train_file']:
        raise ValueError(
            "you must specify dataset with --train-file or --synthetic-data")
