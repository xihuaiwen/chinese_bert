...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if isinstance(config.hidden_act, str) or (sys.version_info[0] == 2 and isinstance(config.hidden_act, unicode)):
            # TODO: Support Gelu. ACT2FN[config.hidden_act]
    ...
    for name, array in zip(names, arrays):
        ...
        for m_name in name:
            ...
            if l[0] == 'cls' and 'squad' in name:
                logger.info("Skipping the cls term to match the torch SQuAD implementation: {}".format(
                    "/".join(name)))
                continue
        ...
        try:
            if pointer is not None and not hasattr(pointer, 'shape'):
                logger.info(
                    "Skipping due to unknown ptr {}".format("/".join(name)))
                continue
            if pointer is None or pointer.shape is None:
                logger.info(
                    "Skipping due to null ptr {}".format("/".join(name)))
                continue
            if m_name == "word_embeddings" and pointer.shape != array.shape:
                logger.warning(
                    f"Cropping the vocabulary for torch load: From {array.shape[0]} to {config.vocab_length}")
