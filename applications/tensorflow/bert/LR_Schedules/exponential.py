...
class LearningRate:
    ...
    def feed_dict_lr(self, step, sess):
        with sess.graph.as_default():
            learing_rate = tf.train.exponential_decay(learning_rate=self.init_lr,
...
def add_arguments(parser):
    lr_group = parser.add_argument_group('Exponential Decay Learning Rate. Use with --lr-schedule exponential.')
    lr_group.add_argument('--warmup-steps', type=float, default=0,
                          help="Warmup length in steps (Default=0, set to 0 for no warmup)")
    lr_group.add_argument('--decay-steps', type=int, default=2000,
    ...
    lr_group.add_argument('--decay-rate', type=float, default=0.9,
                          help="The rate to decay (Default=0.9)")
    return parser
...
def set_defaults(opts):
    ...
    opts['summary_str'] += "Exponential decay applied to learning rate with exponent {decay_rate}."
